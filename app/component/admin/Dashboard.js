                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center text-gray-600">
                        ...
                        <span>{companyData.contactPhone || 'Not provided'}</span>
                      </div>
                      <div className="flex items-center text-gray-600">
                        <Calendar className="h-5 w-5 mr-3" />
                        <span>Registered {new Date(companyData.created_at).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                        })}</span>
                    ...
                    <div className="space-y-4">
                      {ratings
                        ...
                        .map((rating) => {
                          ...
                          const values = Object.values(ratingObj).filter(val => typeof val === 'number');
                          const avgScore = values.length > 0 ? values.reduce((a, b) => a + b) / values.length : 0;
                          return (
                            <div key={rating.id} className="border border-gray-200 rounded-lg p-4">
                              <div className="flex justify-between items-start mb-2">
                                <div>
                                  <p className="font-medium text-gray-900">{rating.username.toUpperCase()}</p>
                                  <p className="text-sm font-medium text-gray-300">
                                    {rating.service_point.toUpperCase()}
                                  </p>
                                </div>
                              ...
                              {rating.rating && (
                                <div className="text-gray-700 text-sm space-y-1">
                                  {Object.entries(typeof rating.rating === 'string' ? JSON.parse(rating.rating) : rating.rating).map(([key, value]) => (
                    ...
                    <div className="space-y-4">
                      {recentComments.map((comment, index) => (
                        <div key={index} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex justify-between items-start mb-2">
                            <p className='flex items-start px-2 py-1 text-xs font-medium text-gray-400'><Phone className='size-4 mr-2 text-purple-600' />{comment.phone_number}</p>
                          ...
                          <ul className="mb-2">
                            {comment.categories.map((cat, i) => (
                              <li key={i}>
                                ...
                                <span className="ml-2 text-gray-700 text-sm">{cat.content}</span>
                              </li>
                            ))}
                          </ul>
                          <p className="text-xs text-gray-500 mt-2">
