...
function ServiceScreen() {
  ...
  useEffect(() => {
    const servicePoints_Criteria = localStorage.getItem("cachedServicePoints");
    if (servicePoints_Criteria) {
      const parsed = JSON.parse(servicePoints_Criteria);
  ...
  const criteriaList = (() => {
    const rawList =
      selectedService?.ratingCriteria?.map((item) => item?.criteriaTitle) ||
      fallbackCriteria;
    const filtered = rawList.filter((criterion) => criterion?.toLowerCase() !== "other");
            ...
            <div className="text-center overflow-y-auto max-h-[75vh] pt-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 max-w-4xl place-items-center">
                  {criteriaList?.map((criterion, index) => (
                    <button
                      ...
                      className={`py-4 px-4 rounded-2xl font-semibold text-base transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 relative w-full text-center ${selectedCriteria.includes(criterion)
              ...
              <div className="flex justify-between items-center max-w-lg mx-auto pt-4 mb-6">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-3 px-8 py-3 rounded-xl text-lg font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 ${selectedCriteria.length > 0
          ...
          <div className="bg-white/95 backdrop-blur-lg rounded-2xl p-8 max-w-md w-full relative shadow-2xl border border-white/20">
              ...
              <div className="flex gap-2 justify-center">
                {[1, 2, 3, 4, 5].map((star) => (
                  <FaStar
                    key={star}
                    size={32}
                    className={`cursor-pointer transition-all duration-200 hover:scale-110 ${star <= otherRating
                      ? "text-yellow-400"
            ...
            <div className="flex justify-between gap-4">
              ...
              <button
                ...
                className={`px-8 py-3 rounded-xl font-semibold text-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl ${otherValue.trim() && otherRating > 0 && otherReason.trim()
                  ? "bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white"
                  : "bg-slate-300 text-slate-500 cursor-not-allowed"
                  }`}
              >
